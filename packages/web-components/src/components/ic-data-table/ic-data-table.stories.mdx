...
To add additonal information about the data held in your ic-data-table, you can use the `title-bar` slot to add an `ic-title-bar`, which will appear above the column headers.
...
The `ic-title-bar` has a built-in density selector, which allows a user to quickly change the size and padding of cells to make it more readable. This can also be hidden if preferred.

Full documentation for this component can be found in the code examples located <ic-link href="/?path=/story/web-components-title-bar--basic" inline="">here</ic-link>.
