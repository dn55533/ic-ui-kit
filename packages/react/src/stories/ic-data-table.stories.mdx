...
A data table can have its rows use custom heights, to help display more data per row. This involves using the `rowHeight` prop, which applies this custom height to all rows.

For a more granular approach, `getRowHeight` exists. This is passed through the data object for each row of data, as well as that row's `index`. Using these properties, you can write a custom function that will return a number for that row's height based on your criteria.
Returning `null` from this function means that the row will use the `rowHeight` property value.
...
- `Dense` = 80% of `rowHeight` or `getRowHeight`
- `Spacious` = 120% of `rowHeight` or `getRowHeight`

E.G: If `rowHeight` = 100, a dense table will set the height to be **_80_**.

To quickly reset all row heights to the default, you can run the `resetRowHeights` method.

If you'd rather let the row calculate its own height based on its content, either set `rowHeight` to `auto`, or return `auto` from `getRowHeight`.
...
<Canvas withSource="none">
  <Story name="Custom Row Heights">
    <IcDataTable
      ...
      data={DATA}
      rowHeight={80}
      getRowHeight={({ firstName, lastName }) => firstName === "Joe" || lastName === "Owens" ? 200 : null}
    />
  </Story>
</Canvas>
